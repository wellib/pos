...
class EShoppingCart extends CMap {
    ...
    public function init(){
        ...
        $CurrentCheck = Yii::app()->getUser()->getState('CurrentCheck', 1); //ID CartId real
        $this->cartId = 'CartID'.$CurrentCheck;
     ...
     */
    public function put(IECartPosition $position, $quantity = 1, $price = 0) {
        $key = $position->getId();
        if ($this->itemAt($key) instanceof IECartPosition) {
            ...
            if ($price == 0)
                $price = $position->getPriceS();
        $this->update($position, $quantity, $price);
     ...
     * @param int $quantity
     */
    public function update(IECartPosition $position, $quantity, $price, $roll = 0, $cart_id = 0) {
